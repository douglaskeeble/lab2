...
(function(global, factory) {
    ...
    arcs.forEach(function(i) {
      ...
      if (f = fragmentByEnd[start]) {
        ...
        if (g = fragmentByStart[end]) {
          ...
          fragmentByStart[fg.start = f.start] = fragmentByEnd[fg.end =
            g.end] = fg;
        } else {
        ...
        if (g = fragmentByEnd[start]) {
          ...
          fragmentByStart[gf.start = g.start] = fragmentByEnd[gf.end =
            f.end] = gf;
        } else {
          fragmentByStart[f.start] = fragmentByEnd[f.end] = f;
        }
        ...
        f = [i];
        fragmentByStart[f.start = start] = fragmentByEnd[f.end =
          end] = f;
      }
    });
  ...
  function meshArcs(topology, object$$1, filter) {
    var arcs, i, n;
    if (arguments.length > 1) arcs = extractArcs(topology, object$$1,
      filter);
    else
  ...
  function extractArcs(topology, object$$1, filter) {
    ...
    geomsByArc.forEach(filter == null ? function(geoms) {
      ...
      if (filter(geoms[0].g, geoms[geoms.length - 1].g)) arcs.push(
        geoms[0].i);
    });
      ...
      polygon.forEach(function(ring) {
        ring.forEach(function(arc) {
          (polygonsByArc[arc = arc < 0 ? ~arc : arc] || (
            polygonsByArc[arc] = [])).push(polygon);
        });
      });
    ...
    polygons.forEach(function(polygon) {
      if (!polygon._) {
        ...
        while (polygon = neighbors.pop()) {
          ...
          polygon.forEach(function(ring) {
            ring.forEach(function(arc) {
              polygonsByArc[arc < 0 ? ~arc : arc].forEach(
                function(polygon) {
                  if (!polygon._) {
                    polygon._ = 1;
    ...
    return {
      ...
      arcs: groups.map(function(polygons) {
        ...
        polygons.forEach(function(polygon) {
          polygon.forEach(function(ring) {
            ring.forEach(function(arc) {
              if (polygonsByArc[arc < 0 ? ~arc : arc].length <
                2) {
                arcs.push(arc);
              }
            });
  ...
  var neighbors = function(objects) {
    ...
    function geometry(o, i) {
      if (o.type === "GeometryCollection") o.geometries.forEach(
        function(o) {
          geometry(o, i);
        });
    ...
    for (var i in indexesByArc) {
      for (var indexes = indexesByArc[i], m = indexes.length, j = 0; j <
        m; ++j) {
        for (var k = j + 1; k < m; ++k) {
          var ij = indexes[j],
            ik = indexes[k],
            n;
          if ((n = neighbors[ij])[i = bisect(n, ik)] !== ik) n.splice(i,
            0, ik);
          if ((n = neighbors[ik])[i = bisect(n, ij)] !== ij) n.splice(i,
            0, ij);
        }
      }
    }
  ...
  var quantize = function(topology, transform) {
    ...
    if (!transform || !transform.scale) {
      if (!((n = Math.floor(transform)) >= 2)) throw new Error(
        "n must be \u22652");
      ...
      var x0 = box[0],
        ...
        n;
      transform = {
        scale: [x1 - x0 ? (x1 - x0) / (n - 1) : 1, y1 - y0 ? (y1 - y0) /
          (n - 1) : 1
        ],
        translate: [x0, y0]
      };
  ...
  var bounds = function(objects) {
    ...
    function boundGeometry(geometry) {
      if (geometry != null && boundGeometryType.hasOwnProperty(geometry
          .type)) boundGeometryType[geometry.type](geometry);
    }
  ...
  var hashset = function(size, hash, equal, type, empty) {
    ...
    var store = new type(size = 1 << Math.max(4, Math.ceil(Math.log(
        size) / Math.LN2))),
      mask = size - 1;
  ...
  var hashmap = function(size, hash, equal, keyType, keyEmpty, valueType) {
    ...
    var keystore = new keyType(size = 1 << Math.max(4, Math.ceil(Math.log(
        size) / Math.LN2))),
  ...
  var join = function(topology) {
    ...
    for (i = 0, n = lines.length; i < n; ++i) {
      ...
      while (++lineStart <= lineEnd) {
        sequence(i, previousIndex = currentIndex, currentIndex =
          nextIndex, nextIndex = indexes[lineStart]);
      }
    ...
    for (i = 0, n = rings.length; i < n; ++i) {
      ...
      while (++ringStart <= ringEnd) {
        sequence(i, previousIndex = currentIndex, currentIndex =
          nextIndex, nextIndex = indexes[ringStart]);
      }
    }
    ...
    function index() {
      var indexByPoint = hashmap(coordinates.length * 1.4, hashIndex,
          equalIndex, Int32Array, -1, Int32Array),
    ...
    var junctionByPoint = hashset(junctionCount * 1.4, hashPoint,
        equalPoint),
      j;
      ...
      while (++ringMid < ringEnd) {
        if (junctions.has(coordinates[ringMid])) {
          if (ringFixed) {
            ...
            rotateArray(coordinates, ringStart, ringEnd, ringEnd -
              ringMid);
  ...
  function reverse$1(array, start, end) {
    for (var mid = start + ((end-- - start) >> 1), t; start < mid; ++
      start, --end) {
  ...
  var dedup = function(topology) {
    ...
    function equalRing(arcA, arcB) {
      ...
      for (var i = 0; i < n; ++i) {
        if (!equalPoint(coordinates[ia + (i + ka) % n], coordinates[ib +
            (i + kb) % n])) return false;
      }
      return true;
    }
    ...
    function reverseEqualRing(arcA, arcB) {
      ...
      for (var i = 0; i < n; ++i) {
        if (!equalPoint(coordinates[ia + (i + ka) % n], coordinates[jb -
            (i + kb) % n])) return false;
      }
      return true;
    }
    ...
    function findMinimumOffset(arc) {
      ...
      while (++mid < end) {
        ...
        if (point[0] < minimumPoint[0] || point[0] === minimumPoint[0] &&
          point[1] < minimumPoint[1]) {
          minimum = mid;
          minimumPoint = point;
        }
      }
  ...
  var delta = function(arcs) {
    ...
    while (++i < n) {
      ...
      while (++j < m) {
        ...
        if (x1 !== x0 || y1 !== y0) arc[k++] = [x1 - x0, y1 - y0], x0 =
          x1, y0 = y1;
      }
  ...
  var extract = function(objects) {
    ...
    function extractLine(line) {
      for (var i = 0, n = line.length; i < n; ++i) coordinates[++index] =
        line[i];
      var arc = {
        0: index - n + 1,
        1: index
    ...
    function extractRing(ring) {
      for (var i = 0, n = ring.length; i < n; ++i) coordinates[++index] =
        ring[i];
      var arc = {
        0: index - n + 1,
        1: index
  ...
  function geomifyObject(input) {
    return input == null ? {
      ...
      input.type === "Feature" ? geomifyFeature : geomifyGeometry)(
      input);
  }
  ...
  var prequantize = function(objects, bbox, n) {
    ...
    function quantizePoint(input) {
      return [Math.round((input[0] - x0) * kx), Math.round((input[1] -
        y0) * ky)];
    }
  ...
  var topology = function(objects, quantization) {
    var bbox = bounds(objects = geometry(objects)),
      transform = quantization > 0 && bbox && prequantize(objects, bbox,
        quantization),
      topology = dedup(cut(extract(objects))),
      coordinates = topology.coordinates,
      indexByArc = hashmap(topology.arcs.length * 1.4, hashArc,
        equalArc);
  ...
  var prune = function(topology) {
    ...
    function scanArc(index) {
      ...
      if (!newIndexByOldIndex[index]) newIndexByOldIndex[index] = 1, ++
        newArcsLength;
    }
    ...
    function reindexArc(oldIndex) {
      return oldIndex < 0 ? ~newIndexByOldIndex[~oldIndex] :
  ...
  var filter = function(topology, filter) {
    ...
    function filterGeometry(input) {
      ...
      switch (input.type) {
        ...
        case "GeometryCollection":
          {
            arcs = input.geometries.map(filterGeometry).filter(
              filterNotNull);
            output = arcs.length ? {
              type: "GeometryCollection",
              geometries: arcs
  ...
  function planarTriangleArea(triangle) {
    var a = triangle[0],
      b = triangle[1],
      c = triangle[2];
    return Math.abs((a[0] - c[0]) * (b[1] - a[1]) - (a[0] - b[0]) * (c[1] -
      a[1])) / 2;
  }
  ...
  var newHeap = function() {
    ...
    heap.pop = function() {
      ...
      var removed = array[0],
        object;
      if (--size > 0) object = array[size], down(array[object._ = 0] =
        object, 0);
      return removed;
    };
    ...
    heap.remove = function(removed) {
      ...
      if (i !== --size) object = array[size], (compare(object,
        removed) < 0 ? up : down)(array[object._ = i] = object, i);
      return i;
    };
    ...
    function down(object, i) {
      while (true) {
        var r = (i + 1) << 1,
          ...
          j = i,
          child = array[j];
        if (l < size && compare(array[l], child) < 0) child = array[j =
          l];
        if (r < size && compare(array[r], child) < 0) child = array[j =
          r];
        if (j === i) break;
  ...
  var presimplify = function(topology, weight) {
    var point = topology.transform ? transform(topology.transform) :
      copy,
      heap = newHeap();
