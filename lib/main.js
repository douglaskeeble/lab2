(function() {
  ...
  function changeAttribute(attribute, csvData) {
    ...
    var bars = d3.selectAll(".bar")
      ...
      .duration(500);
    //storing the max value to change axis size
    var maxV = d3.max(csvData, function(d) {
      return parseFloat(d[expressed])
    });
    // if max value is greater than 20 set to 100%
    if (maxV > 20) {
      maxV = 100
        // else set max to maximum value plus 3
    } else {
      maxV = maxV + 3
    }
    //update the axis
  ...
  function highlight(props) {
    //change stroke
    var i;
    // tracts and data have different geoid names
    if (props.GEOID_Data) {
      i = props.GEOID_Data;
    } else {
      i = props.geoid
  ...
  function moveLabel() {
    ...
    var x = d3.event.clientX > window.innerWidth - labelWidth - 20 ? x2 :
      x1;
